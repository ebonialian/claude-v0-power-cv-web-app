...
function ScoreBadge({ score }: { score: number }) {
  const getColor = (s: number) => {
    if (s >= 75) return "bg-green-500/20 text-green-500"
    if (s >= 50) return "bg-yellow-500/20 text-yellow-500"
...
export default function HistorialPage() {
  ...
  const [remoteAnalyses, setRemoteAnalyses] = useState<AnalysisResult[]>([])
    ...
    const fetchAnalyses = async () => {
      ...
      try {
        const { data, error } = await supabase
          .from("analyses")
          .select(
            "id, created_at, job_target, file_name, score_total, categorias, problemas, sugerencias, resumen, rangos_salariales, cursos_recomendados, oferta_laboral_texto"
          )
          .eq("user_id", user.id)
        ...
        const mapped: AnalysisResult[] =
          (data || []).map((row: any) => ({
            ...
            resumen: row.resumen ?? "",
            rangos_salariales: row.rangos_salariales ?? undefined,
            cursos_recomendados: row.cursos_recomendados ?? undefined,
            ofertaLaboralTexto: row.oferta_laboral_texto ?? undefined,
          })) as AnalysisResult[]

        setRemoteAnalyses(mapped)
      } finally {
        setIsLoading(false)
      }
    }
      ...
      <div className="p-4 sm:p-6 lg:p-8 max-w-4xl mx-auto">
        <div className="mb-8">
          ...
          <p className="text-muted-foreground">
            El historial detallado es una función exclusiva del plan Pro.
          </p>
        </div>
